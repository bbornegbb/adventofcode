...
def find_fastest(G, time, startpos, endpos, cutoff=None):
    nn = node_name(time, *startpos)
    paths = nx.shortest_paths.single_source_dijkstra_path(G, nn, cutoff=cutoff)
    for i in range(time, MAX_ITER + 1):
        name = node_name(i, *endpos)
        if name in paths:
            return i, paths[name]
...
def analyze(fname):
    ...
    for iteration, (f1, f2) in enumerate(
        ...
        if iteration == MAX_ITER:
            break
    t1, path1 = find_fastest(G, 0, start, end, 400)
    print(t1, path1[-1])
    t2, path2 = find_fastest(G, t1, end, start, 400)
    print(t2, path2[-1])
    t3, path3 = find_fastest(G, t2, start, end, 400)
    print(t3, path3[-1])
    return [t1, t2, t3], path3[-1], start, end
